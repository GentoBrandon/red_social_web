...
export default class UserCredentialsController{
    static async createUserCredential(req:Request,res:Response, next:NextFunction):Promise<void>{
        ...
        try{
            ...
            res.status(201).json({message:"User_credentials created successfully",id:resultInsert.id});
        }catch(error){
            next(error);
        }
    }
    static async getAllUserCredentials(req:Request,res:Response,next:NextFunction):Promise<void>{
        try{
