...
export async function up(knex: Knex): Promise<void> {
    ...
    if (!exist) {
        await knex.schema.createTable('request_friends', table => {
            ...
            table.foreign('id_profile_request').references('id').inTable('profiles').onDelete('CASCADE');
            table.foreign('id_profile_response').references('id').inTable('profiles').onDelete('CASCADE');   
