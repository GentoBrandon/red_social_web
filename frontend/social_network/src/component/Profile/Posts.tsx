...
const PostCard: React.FC = () => {
    const [posts, setPosts] = useState<Post[]>([]);
    const [idProfile, setIdProfile] = useState<number | null>(null);
    const { socket } = useSocket(); // Usa el WebSocket desde el contexto

    const toggleLike = (postId: number) => {
        const post = posts.find((p) => p.id === postId);
        if (!post || idProfile === null || !socket) return;

        const isLiked = !post.isLiked;
        socket.emit('toggle-like', { postId, userId: idProfile, isLiked });
    ...
    // Escucha el evento `like-updated` desde el WebSocket
    useEffect(() => {
        if (socket) {
            socket.on('like-updated', ({ postId, userId, isLiked }) => {
                setPosts((prevPosts) =>
                    prevPosts.map((p) =>
                        p.id === postId ? { ...p, isLiked } : p
                    )
        ...
        const fetchAndSetProfileData = async () => {
            try {
                ...
                if (id) {
                    ...
                    const postsWithLikes = response.data.map((post: Post) => ({
                        ...post,
                        isLiked: false, // Inicializa el estado de "like"
                    }));
                    setPosts(postsWithLikes);
                }
            } catch (error) {
